(ns clj-shower.core
    ...
    (:use [clojure.string :only [blank? join trim]])
    (:use hiccup.page)
    (:use hiccup.element))
...
(defn code [content]
  (with-meta [:code content] {:wrapper :pre}))

(defn ul [spaces content]
  (with-meta [:li content] {:wrapper :ul :indent (count spaces)}))

(defn ol [spaces content]
  (with-meta [:li content] {:wrapper :ol :indent (count spaces)}))
   ...
   (into []
         (lazy-seq
           (when-let [s (seq parsed-slide-lines)]
                     (let [[fst & xs] s
                           {wrapper :wrapper indent :indent} (meta fst)]
                       (if wrapper
                         [(into [wrapper] (cons fst (convert xs indent)))]
                         (cons fst (convert xs))))))))
...
(defn converter [parsed-slide-lines]
  (vec (convert parsed-slide-lines)))
